...
def index():
    ...
    toplamlar = {
        "alis": genel_alis, "deger": genel_deger, "kar": genel_kar, "realize_kar": toplam_realize_kar, 
        "hesap_para": hesap_para, "toplam_portfoy": toplam_portfoy_degeri, 
        "kz_yuzde": round((genel_kar / genel_alis * 100) if genel_alis > 0 else 0, 2), 
        "hisse_toplam_deger": hisse_toplam_deger, "hisse_grup_kz": hisse_grup_kz_yuzde, 
        "halka_toplam_deger": halka_toplam_deger, "halka_grup_kz": halka_grup_kz_yuzde, 
        "satis_toplam_lot": satis_toplam_lot, "satis_toplam_deger": satis_toplam_deger, "satis_toplam_kar": satis_toplam_kar
    }
...
# --- HZM BTS: MİLİMETRİK CANLI VERİ & YÜZDE DEĞİŞİM SENSÖRÜ ---
...
def api_canli_piyasa_yenile():
    ...
    # ADIM A: Fintables API Hattını Dene
    try:
        ...
        if 'PYTHONANYWHERE_SITE' in os.environ or os.path.exists('/home/halidmusab'):
        ...
        if r.status_code == 200:
            ...
            for comp in companies_list:
                ...
                # Günlük değişim verisini al ve string/float kontrolü yap (Kritik Düzeltme)
                raw_degisim = comp.get('daily_change') or comp.get('change', 0.0)
                
                if hisse and fiyat is not None:
                    try:
                        # Eğer string gelirse içindeki sembolleri temizle
            ...
            print(f"✅ [HZM BTS] Fintables API verileri mühürlendi. {guncellenen_adet} hisse.")
            
    except Exception as e:
        print(f"⚠️ Fintables API hattı kısıtlandı, HZM B Planı (yfinance) devreye alınıyor. Durum: {e}")

    # ADIM B: Fintables Bloklandıysa yfinance Güvence Katmanını Çalıştır
    if not fintables_basarili:
        try:
            ...
            if not elimizdeki_hisseler:
                elimizdeki_hisseler = ["THYAO", "ASELS", "EREGL", "TUPRS", "ISCTR"]
            ...
            for hisse in elimizdeki_hisseler:
                try:
                    ...
                    # Son 2 günlük veriyi çekiyoruz ki dünkü kapanışı tam bilelim
                    ...
                    if not todays_data.empty:
                        ...
                        # Yahoo Info'dan çekmeyi dene
                        try:
                            if 'regularMarketChangePercent' in ticker.info and ticker.info['regularMarketChangePercent'] is not None:
                                degisim_yuzde = float(ticker.info['regularMarketChangePercent'])
                            else:
                                raise ValueError("Info bos dondu")
                        except:
                            # Matematiksel Oran Güvencesi: (Bugünkü Kapanış - Dünkü Kapanış) / Dünkü Kapanış * 100
                            if len(todays_data) >= 2:
                                dünkü_kapanis = float(todays_data['Close'].iloc[-2])
                                degisim_yuzde = ((kapanis_fiyati - dünkü_kapanis) / dünkü_kapanis) * 100
                        
                        c_db.execute('''INSERT INTO piyasa_fiyatlari (hisse, fiyat, gunluk) VALUES (?, ?, ?)
                                     ON CONFLICT(hisse) DO UPDATE SET fiyat=excluded.fiyat, gunluk=excluded.gunluk''',
                                  (hisse, kapanis_fiyati, round(degisim_yuzde, 2)))
                        guncellenen_adet += 1
                except Exception as yf_err:
                    print(f"⚠️ {hisse} Yahoo üzerinden süzülemedi: {yf_err}")
                    continue
            print(f"✅ [HZM BTS] Yahoo Finance güvence katmanı mühürlendi. {guncellenen_adet} hisse.")
            
        except Exception as final_err:
            print(f"❌ Tüm borsa veri hatları tıkandı: {final_err}")
